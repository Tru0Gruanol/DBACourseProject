...
- `AuthController.java`：两个新接口
  - `POST /api/auth/login`：传入学号/工号/管理员名 + 密码，后端自动查学生表、教师表和管理员，不用前端告诉它"这个人是学生还是老师还是管理员"
...
**登录页**：原来三个标签页（学生/教师/管理员），先改成两个（账号登录 + 管理员登录），最终合并成一个统一的登录表单。学生、教师和管理员共用同一个入口，输入学号/工号/管理员名 + 密码，后端自动识别三种身份。
...
**管理员**：admin / admin123 同样走后端验证，不再前端硬编码校验，登录页面看不到任何管理员密码提示。
